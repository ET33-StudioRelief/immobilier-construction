...
(() => {
  ...
  var animateButtonTertiary = () => {
    ...
    buttons.forEach((button) => {
      ...
      btn.addEventListener("mouseenter", () => {
        gsapWithCSS.to(btn, {
          backgroundImage: "linear-gradient(90deg, #ffffff 0%, #fffefe 85%)",
      ...
      btn.addEventListener("mouseleave", () => {
        gsapWithCSS.to(btn, {
          backgroundImage: "linear-gradient(90deg, #1e303c 0%, #346c75 85%)",
          // Revenir à la couleur initiale du fond
          color: "#ffffff",
          // Revenir à la couleur initiale du texte
          duration: 0.4,
          ease: "power1.inOut"
        });
      });
  ...
  var animateButtonSecondary = () => {
    const buttons = document.querySelectorAll(".button.is-secondary");
    buttons.forEach((button) => {
      const btn = button;
      btn.style.backgroundColor = "#ffffff";
      btn.style.color = "#1e303c";
      btn.style.border = "2px solid #1e303c";
      ...
      btn.addEventListener("mouseleave", () => {
        gsapWithCSS.fromTo(
          ...
          {
            backgroundImage: "linear-gradient(90deg, #1e303c 0%, #346c75 85%)",
            // Valeur initiale pour l'animation
            color: "#ffffff"
            // Valeur initiale pour le texte
          },
          {
            backgroundImage: "linear-gradient(90deg, #ffffff 0%, #fffefe 85%)",
            // Nouveau dégradé de blanc
            backgroundColor: "#ffffff",
            // Revenir à la couleur initiale du fond
            color: "#1e303c",
            // Revenir à la couleur initiale du texte
            duration: 0.6,
            // Durée de l'animation de sortie pour plus de douceur
            ease: "power1.out"
  ...
  function updateFooterYear() {
    ...
    if (footerYearElement) {
      footerYearElement.textContent = currentYear.toString();
    }
  }
  var animateSectionsSlideRight = () => {
    const sections = [".section_lp-testimonial", ".section_slider-galerie"];
    sections.forEach((selector3) => {
      if (selector3 === ".section_lp-testimonial" && window.innerWidth <= 767)
        return;
      gsapWithCSS.from(selector3, {
        x: 100,
        // Commence 100px à droite de la position finale
        opacity: 0,
        // Commence invisible
        duration: 3,
        // Durée de l'animation
    ...
    items.forEach((item) => {
      ...
      if (image) {
        gsapWithCSS.fromTo(
          image,
          { y: -100 },
          // Position initiale de l'image (parallax à -50px)
          {
            y: 100,
            // La valeur finale lorsque l'élément est visible
            ease: "none",
            // Effet linéaire pour un parallax fluide
            scrollTrigger: {
              trigger: item,
              // Utiliser chaque div hp-services_item comme déclencheur
              start: "top bottom",
              // Démarre lorsque le haut de l'élément atteint le bas du viewport
              end: "bottom top",
              // Fin lorsque le bas de l'élément atteint le haut du viewport
              scrub: true
              // Liaison avec le scroll pour un effet fluide
            }
          }
        );
      }
    ...
    items.forEach((item) => {
      ...
      if (leftImages.length) {
        gsapWithCSS.fromTo(
          ...
          { y: 0 },
          {
            ...
            // Effet linéaire pour un parallax fluide
            scrollTrigger: {
              trigger: ".lp-galery_list",
              // Utiliser chaque div lp-galery_lightbox-link comme déclencheur
              start: "top bottom",
              // Démarre lorsque le haut de l'élément atteint le bas du viewport
              end: "bottom 200",
              // Fin lorsque le bas de l'élément atteint le haut du viewport
      ...
      if (centerImages.length) {
        gsapWithCSS.fromTo(
          centerImages,
          { y: 0 },
          // Position initiale de l'image (parallax à -100px)
          {
            y: 125,
            // La valeur finale lorsque l'élément est visible
            ease: "none",
      ...
      if (element) {
        gsapWithCSS.from(selector3, {
          ...
          scrollTrigger: {
            ...
            start: "top 80%",
            toggleActions: "play reverse play reverse"
            // Jouer et inverser l'animation à l'entrée et à la sortie
          }
        });
      }
    });
  };
  var animateFeaturesItems = () => {
    const featuresItems = document.querySelectorAll(".lp-features_item");
    featuresItems.forEach((item) => {
      gsapWithCSS.from(item, {
        y: 50,
        // L'élément commence 50px plus bas que sa position finale
        opacity: 0,
        // Commence invisible
        duration: 2,
        // Durée de l'animation
  ...
  var animateSectionsHistory = () => {
    gsapWithCSS.from(".section_history", {
      y: 50,
      // L'élément commencera 50px plus bas que sa position finale
      opacity: 0,
      // Commence invisible
  ...
  function animateCounter() {
    ...
    ScrollTrigger2.create({
      ...
      onEnter: () => {
        countObj.count = 0;
        gsapWithCSS.to(countObj, {
          count: 300,
          duration: 2,
          ease: "power3.out",
          onUpdate: () => {
            statsElement.textContent = `+${Math.round(countObj.count)}`;
          },
          onComplete: () => {
            statsElement.textContent = "+300";
          }
        });
      }
    });
  ...
  function animatePercentageCounter() {
    ...
    if (!percElement)
      return;
    const percObj = { percent: 0 };
    ScrollTrigger2.create({
      trigger: ".section_stats",
      // Utilisez le même trigger pour que cela fonctionne avec la section appropriée
      start: "top 80%",
      onEnter: () => {
        percObj.percent = 0;
        gsapWithCSS.to(percObj, {
          percent: 100,
          duration: 2,
          ease: "power3.out",
          onUpdate: () => {
            percElement.textContent = `+${Math.round(percObj.percent)}%`;
          },
          onComplete: () => {
            percElement.textContent = "+100%";
          }
        });
      }
    });
  ...
  var leftContent = document.querySelector(".guaranteest_content-left");
  if (leftContent && window.innerWidth > 767) {
    leftContent.style.position = "sticky";
    leftContent.style.top = "50px";
  }
})();
